...
class RenderNovelView:
    ...
    """Path to dataset."""
    output: Path = Path('./render_out_patches/')
    ...
    model_checkpoint: Path = Path('outputs/pixelnerf-mlp-stage-one-sv-ko/pixelnerf-mlp/2023-06-26_083301/nerfstudio_models/step-000500000.ckpt')
    """model checkpoint"""
    # encoder_checkpoint: Path = Path('outputs/stage-one-km-sv-with-mask/car-nerf/2023-06-18_162045/nerfstudio_models/step-000529000.ckpt')
    encoder_checkpoint: Path = Path('outputs/pixelnerf-mlp-stage-one-sv-ko/pixelnerf-mlp/2023-06-26_083301/nerfstudio_models/step-000500000.ckpt')
    """encoder checkpoint"""
    model_config: ModelConfig = PixelNerfMLPConfig(background_color='white')
    """model config"""
    encoder_config: ModelConfig = PixelNerfMLPConfig(background_color='white')
    """model config"""
    ...
    def main(self) -> None:
        ...
        self._save_origins(self.all_objects)

        
        
        # unoptimized_frames = self.all_objects
        
        # self._generate_all_seq(unoptimized_frames,
        #                         device)
    ...
    def _init_encoder(self, device:str = 'cuda') -> torch.nn.Module:
        ...
        scene_box = SceneBox(torch.tensor([[-1., -1., -1.],
                    [1.0, 1.0, 1.0]]))
        device = 'cuda'
        new_model = PixelNerfMLPModel(self.encoder_config, scene_box, num_train_data=114515).to(device)
    ...
    def _parse_all_instances(self):
        ...
        for meta_all in meta_list:
            instances_all = meta_all['instances']
            instance_list = instance_list + instances_all

        num_images = len(instance_list)
        ...
        return [instance_list[i] for i in i_eval]
    ...
    def _apply_mask_and_get_latents(self, mask, instance:Dict, device, color_name) -> torch.Tensor:
        with torch.no_grad():
            ...
            self._save_img(patch_tensor, Path('./test.png'))
            patch_tensor = self.img_transform(patch_tensor.permute(2,0,1)).permute(1,2,0).to(device)
    ...
    def _generate_all_seq(self, unoptimized_frames, device):
        for instance in unoptimized_frames:
            ...
            if 'rgb_fine' in out_img:
                patch = out_img['rgb_fine']
            else:
                patch = out_img['rgb']
            if 'accumulation_fine' in out_img:
                mask = out_img['accumulation_fine']
            else:
                mask = out_img['accumulation']
            color_mask = colormaps.apply_colormap(mask)
            file_name = instance['patch'].split('/')[-1]
            self._save_img(patch,
                self.output / f'patch_{file_name}')
            self._save_img(color_mask,
                self.output / f'mask_{file_name}')
    ...
    def _save_origins(self, instances):
        for instance in instances:
            patch_file = str((self.data / instance['patch']).absolute())
            target_file = str((self.output / patch_file.rsplit('/',maxsplit=1)[-1]).absolute())
