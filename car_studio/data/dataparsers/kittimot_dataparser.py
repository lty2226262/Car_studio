...
@dataclass
class KittiMotDataParserConfig(DataParserConfig):
    """kittimot dataset config"""
    ...
    """Sequence id"""
    start_id: int = 72
    """Start id, -1 for the very start frame"""
    end_id: int = 138
    """End id, -1 for the very last frame"""
...
@dataclass
class KittiMot(DataParser):
    """KittiMot DatasetParser"""
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        meta_list = []
        meta = load_from_json(self.config.data / ("km.json"))
        ...
        for meta_all in meta_list:
            ...
            for instance_it in meta_all["instances"]:
                if int(instance_it['patch'][9:11]) != self.config.sequence_num:
                    continue
                fid = int(instance_it['patch'][11:16])
                if self.config.start_id != -1 and self.config.start_id > fid:
                    continue
                if self.config.end_id != -1 and self.config.end_id < fid:
                    continue
        ...
        has_split_files_spec = any(f"{split}_filenames" in meta_list for split in ("train", "val", "test"))
        if f"{split}_filenames" in meta_list:
            # Validate split first
            split_filenames = set(self._get_fname(Path(x), data_dir) for x in meta_list[f"{split}_filenames"])
            ...
            i_train = i_all# equally spaced training images starting and ending at 0 and num_images-1
            i_eval = i_all
            # i_eval = np.setdiff1d(i_all, i_train)  # eval images are the remaining images
            assert len(i_eval) == num_images
            if split == "train":
                indices = i_train
    ...
    def _parse_labels(self, path: Path, seq_id: int, start_id : int, end_id : int):
        tracking_label_path = str(path.absolute()) + f'training/label_02/{seq_id:04d}.txt'
        with open(tracking_label_path) as f:
            lines = f.readlines()
        instance_list = []
        for line in lines:
            instance_it = {}
...
KittiMOTDataParserConfigSpecification = DataParserSpecification(config=KittiMotDataParserConfig)
