...
from car_studio.fields.carnerf_field import CarNeRFField
from car_studio.model_components.custom_ray_samplers import (
    CustomPDFSampler, CustomUniformSampler)


@dataclass 
class CarNerfModelConfig(VanillaModelConfig):
    """Car Nerf Config"""

    _target: Type = field(default_factory=lambda: CarNerfModel)
    background_color: Literal["random", "black", "white"] = "random"
    enable_collider: bool = False

class CarNerfModel(Model):
    """Car Nerf model
    ...
    def populate_modules(self):
        ...
        self.field = CarNeRFField(
            position_encoding=position_encoding,
            direction_encoding=direction_encoding, use_integrated_encoding=True
        )
    ...
    def get_outputs(self, ray_bundle: RayBundle) -> Dict[str, Tensor]:
        ...
        if torch.isnan(rgb_fine[fine_intersect, ...]).sum():
            print('debug here')
        return outputs
