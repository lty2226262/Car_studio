...
@dataclass
class BaseStageTwoPipelineConfig(VanillaPipelineConfig):
    """Base Stage Two Pipeline Config"""

    _target: Type = field(default_factory=lambda: BaseStageTwoPipeline)
...
class BaseStageTwoPipeline(Pipeline):
    ...
    def __init__(
        self,
        config: BaseStageTwoPipelineConfig,
        device: str,
        ...
        ), "CarNerfStageTwoPipeline only works with CarPatchDataManager."
        ...
        self.global_latent_codes_lookup_table = self.init_latent_codes(device)
    ...
    def get_train_loss_dict(self, step: int):
        ...
        indices_list = []
        obj_ids_list = []
        latents = torch.empty((0, 512),device=ray_bundle.origins.device)
        for idx, _ in batch['patch'].items():
            indices_list.append(idx)
            obj_id = batch['object_id'][idx]
            obj_ids_list.append(obj_id)
            latents = torch.cat([latents,
                                 self.global_latent_codes_lookup_table[obj_id]], 
                                 dim=0)
        ...
        for ori_id, _ in batch["patch"].items():
    ...
    def get_eval_loss_dict(self, step: int):
        ...
        object_id = list(batch['object_id'].values())[0]

        latents = self.global_latent_codes_lookup_table[object_id]
    ...
    def get_eval_image_metrics_and_images(self, step: int):
        ...
        latents = self.global_latent_codes_lookup_table[batch['object_id'][0]]
        camera_ray_bundle.metadata['latents'] = latents

        outputs = self.model.get_outputs_for_camera_ray_bundle_and_latents(camera_ray_bundle)
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        globel_latent_codes = {'latent_vectors': \
                               list(self.global_latent_codes_lookup_table.values())}
        
        if 'encoder' in model_params.keys():
            del self.model.encoder
            model_params.pop('encoder')

        # TODO(ethan): assert that key names don't overlap
        return {**datamanager_params, **model_params, **globel_latent_codes}

    @abstractmethod
    def init_latent_codes(self, device) -> Dict:
        """Blank init_latent_codes method
        """
        raise NotImplementedError
